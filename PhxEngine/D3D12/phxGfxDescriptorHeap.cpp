	...
	D3D12_DESCRIPTOR_HEAP_FLAGS flags)
{
	...
	this->m_descriptorSize = device->GetPlatformDevice()->GetDescriptorHandleIncrementSize(type);
...
#pragma warning( disable : 4267)
	auto newPage = std::make_shared<DescriptorHeapAllocationPage>(
		...
		this,
		this->m_device->GetPlatformDevice2(),
		this->m_heapDesc,
...
{
	this->m_device = device;
	this->m_descriptorSize = this->m_device->GetPlatformDevice()->GetDescriptorHandleIncrementSize(type);
	...
	ThrowIfFailed(
		this->m_device->GetPlatformDevice()->CreateDescriptorHeap(
			&this->m_heapDesc,
