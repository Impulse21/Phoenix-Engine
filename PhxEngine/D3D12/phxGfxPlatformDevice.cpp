...
namespace
{
	...
	constexpr D3D12_SUBRESOURCE_DATA _ConvertSubresourceData(const gfx::SubresourceData& pInitialData)
	{
...
gfx::D3D12Device::D3D12Device()
{
	assert(gfx::D3D12Device::Ptr == nullptr);
	gfx::D3D12Device::Ptr = this;
}

gfx::D3D12Device::~D3D12Device()
{
	gfx::D3D12Device::Ptr = nullptr;
}

void gfx::D3D12Device::Initialize()
{
	PHX_CORE_INFO("Initialize DirectX 12 Graphics Device");
	this->InitializeResourcePools();
	this->InitializeD3D12NativeResources(this->m_gpuAdapter.PlatformAdapter.Get());
...
void gfx::D3D12Device::SubmitFrame()
{
#if false
	// Submit Commandlists
	...
	for (uint32_t iCmd = 0; iCmd < numActiveCommandLists; ++iCmd)
	{
		...
		if (dependency)
		{
			...
			for (auto& wait : commandList.Waits)
			{
				...
				ThrowIfFailed(
					this->GetQueue(waitCommandList.QueueType).m_platformQueue.Get()->Wait(
	...
	{
		UINT presentFlags = 0;
		if (!swapChainImpl.Desc.VSync && !swapChainImpl.Desc.Fullscreen)
		{
			presentFlags = DXGI_PRESENT_ALLOW_TEARING;
		}
		HRESULT hr = swapChainImpl.NativeSwapchain4->Present((UINT)swapChainImpl.Desc.VSync, presentFlags);
	...
	{
		const uint32_t bufferCount = swapChainImpl.Desc.BufferCount;
...
void gfx::D3D12Device::WaitForIdle()
{
	...
	{
		D3D12CommandQueue& queue = this->m_commandQueues[q];
		hr = queue.m_platformQueue.Get()->Signal(fence.Get(), 1);
		assert(SUCCEEDED(hr));
...
bool gfx::D3D12Device::IsHdrSwapchainSupported()
{
	if (!swapChainImpl.NativeSwapchain)
	{
		return false;
	}
	...
	if (SUCCEEDED(swapChainImpl.NativeSwapchain->GetContainingOutput(&dxgiOutput)))
	{
...
CommandSignatureHandle gfx::D3D12Device::CreateCommandSignature(CommandSignatureDesc const& desc, size_t byteStride)
{
...
void gfx::D3D12Device::DeleteCommandSignature(CommandSignatureHandle handle)
{
	if (!handle.IsValid())
	{
		return;
...
ShaderHandle gfx::D3D12Device::CreateShader(ShaderDesc const& desc, Span<uint8_t> shaderByteCode)
{
...
InputLayoutHandle gfx::D3D12Device::CreateInputLayout(VertexAttributeDesc* desc, uint32_t attributeCount)
{
...
GfxPipelineHandle gfx::D3D12Device::CreateGfxPipeline(GfxPipelineDesc const& desc)
{
...
void gfx::D3D12Device::DeleteGfxPipeline(GfxPipelineHandle handle)
{
	if (!handle.IsValid())
	{
		return;
...
ComputePipelineHandle gfx::D3D12Device::CreateComputePipeline(ComputePipelineDesc const& desc)
{
...
MeshPipelineHandle gfx::D3D12Device::CreateMeshPipeline(MeshPipelineDesc const& desc)
{
...
void gfx::D3D12Device::DeleteMeshPipeline(MeshPipelineHandle handle)
{
	if (!handle.IsValid())
	{
		return;
...
RenderPassHandle gfx::D3D12Device::CreateRenderPass(RenderPassDesc const& desc)
{
...
void gfx::D3D12Device::GetRenderPassFormats(RenderPassHandle handle, std::vector<gfx::Format>& outRtvFormats, gfx::Format& depthFormat)
{
	D3D12RenderPass* renderPass = this->GetRenderPassPool().Get(handle);
	if (!renderPass)
	{
		depthFormat = gfx::Format::UNKNOWN;
		return;
	}
	...
	{
		gfx::Format format = this->GetTextureDesc(attachment.Texture).Format;
...
gfx::RenderPassDesc gfx::D3D12Device::GetRenderPassDesc(RenderPassHandle handle)
{
	D3D12RenderPass* renderPass = this->GetRenderPassPool().Get(handle);
	return renderPass->Desc;
}

void gfx::D3D12Device::DeleteRenderPass(RenderPassHandle handle)
{
	if (!handle.IsValid())
	{
		return;
...
TextureHandle gfx::D3D12Device::CreateTexture(TextureDesc const& desc)
{
	...
	const bool isTypeless = (desc.MiscFlags | gfx::TextureMiscFlags::Typeless) == gfx::TextureMiscFlags::Typeless;
	...
	if ((desc.BindingFlags & BindingFlags::ShaderResource) == BindingFlags::ShaderResource)
	{
		this->CreateSubresource(texture, gfx::SubresouceType::SRV, 0, ~0u, 0, ~0u);
	}

	if ((desc.BindingFlags & BindingFlags::RenderTarget) == BindingFlags::RenderTarget)
	{
		this->CreateSubresource(texture, gfx::SubresouceType::RTV, 0, ~0u, 0, ~0u);
	}

	if ((desc.BindingFlags & BindingFlags::DepthStencil) == BindingFlags::DepthStencil)
	{
		this->CreateSubresource(texture, gfx::SubresouceType::DSV, 0, ~0u, 0, ~0u);
	}

	if ((desc.BindingFlags & BindingFlags::UnorderedAccess) == BindingFlags::UnorderedAccess)
	{
		this->CreateSubresource(texture, gfx::SubresouceType::UAV, 0, ~0u, 0, ~0u);
	}

	return texture;
}

const TextureDesc& gfx::D3D12Device::GetTextureDesc(TextureHandle handle)
{
	const D3D12Texture* texture = this->m_texturePool.Get(handle);
	return texture->Desc;
}

DescriptorIndex gfx::D3D12Device::GetDescriptorIndex(TextureHandle handle, SubresouceType type, int subResource)
{
	const D3D12Texture* texture = this->m_texturePool.Get(handle);
...
void gfx::D3D12Device::DeleteTexture(TextureHandle handle)
{
	if (!handle.IsValid())
	{
		return;
...
BufferHandle gfx::D3D12Device::CreateBuffer(BufferDesc const& desc)
{
	...
	{
		this->CreateSubresource(buffer, gfx::SubresouceType::SRV, 0u);
	}

	if ((desc.Binding & BindingFlags::UnorderedAccess) == BindingFlags::UnorderedAccess)
	{
		this->CreateSubresource(buffer, gfx::SubresouceType::UAV, 0u);
	}

	return buffer;
}

const BufferDesc& gfx::D3D12Device::GetBufferDesc(BufferHandle handle)
{
	return this->m_bufferPool.Get(handle)->Desc;
}

DescriptorIndex gfx::D3D12Device::GetDescriptorIndex(BufferHandle handle, SubresouceType type, int subResource)
{
...
void* gfx::D3D12Device::GetBufferMappedData(BufferHandle handle)
{
	const D3D12Buffer* bufferImpl = this->m_bufferPool.Get(handle);
...
uint32_t gfx::D3D12Device::GetBufferMappedDataSizeInBytes(BufferHandle handle)
{
	const D3D12Buffer* bufferImpl = this->m_bufferPool.Get(handle);
	assert(bufferImpl->GetDesc().Usage != Usage::Default);
...
void gfx::D3D12Device::DeleteBuffer(BufferHandle handle)
{
	if (!handle.IsValid())
	{
		return;
...
RTAccelerationStructureHandle gfx::D3D12Device::CreateRTAccelerationStructure(RTAccelerationStructureDesc const& desc)
{
		...
		for (auto& g : desc.ButtomLevel.Geometries)
		{
			...
			if (g.Type == RTAccelerationStructureDesc::BottomLevelDesc::Geometry::Type::Triangles)
			{
				...
				trianglesDesc.IndexBuffer =
					...
					(D3D12_GPU_VIRTUAL_ADDRESS)g.Triangles.IndexOffset * (g.Triangles.IndexFormat == gfx::Format::R16_UINT ? sizeof(uint16_t) : sizeof(uint32_t));
				trianglesDesc.IndexCount = g.Triangles.IndexCount;
				trianglesDesc.IndexFormat = g.Triangles.IndexFormat == gfx::Format::R16_UINT ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
...
const RTAccelerationStructureDesc& gfx::D3D12Device::GetRTAccelerationStructureDesc(RTAccelerationStructureHandle handle)
{
	assert(handle.IsValid());
	return this->m_rtAccelerationStructurePool.Get(handle)->Desc;
}

void gfx::D3D12Device::WriteRTTopLevelAccelerationStructureInstance(RTAccelerationStructureDesc::TopLevelDesc::Instance const& instance, void* dest)
{
...
size_t gfx::D3D12Device::GetRTTopLevelAccelerationStructureInstanceSize()
{
	return sizeof(D3D12_RAYTRACING_INSTANCE_DESC);
}

void gfx::D3D12Device::DeleteRtAccelerationStructure(RTAccelerationStructureHandle handle)
{
	if (!handle.IsValid())
	{
		return;
...
DescriptorIndex gfx::D3D12Device::GetDescriptorIndex(RTAccelerationStructureHandle handle)
{
...
int gfx::D3D12Device::CreateSubresource(TextureHandle texture, SubresouceType subresourceType, uint32_t firstSlice, uint32_t sliceCount, uint32_t firstMip, uint32_t mipCount)
{
...
int gfx::D3D12Device::CreateSubresource(BufferHandle buffer, SubresouceType subresourceType, size_t offset, size_t size)
{
	switch (subresourceType)
	{
	case SubresouceType::SRV:
...
#if false
TimerQueryHandle gfx::D3D12Device::CreateTimerQuery()
{
...
void gfx::D3D12Device::DeleteTimerQuery(TimerQueryHandle query)
{
	if (!query.IsValid())
	{
		return;
...
bool gfx::D3D12Device::PollTimerQuery(TimerQueryHandle query)
{
...
TimeStep gfx::D3D12Device::GetTimerQueryTime(TimerQueryHandle query)
{
	assert(query.IsValid());
	D3D12TimerQuery* queryImpl = this->m_timerQueryPool.Get(query);
	...
	uint64_t frequency;
	this->GetGfxQueue().m_platformQueue.Get()->GetTimestampFrequency(&frequency);
...
void gfx::D3D12Device::ResetTimerQuery(TimerQueryHandle query)
{
	assert(query.IsValid());
	D3D12TimerQuery* queryImpl = this->m_timerQueryPool.Get(query);
...
inline TextureHandle gfx::D3D12Device::GetBackBuffer(SwapChainHandle handle)
{
	D3D12SwapChain* swapChain = this->m_swapChainPool.Get(handle);

	if (!swapChain)
		return {};

	// TODO:
	return swapChain->BackBuffers[this->GetBackBufferIndex()];
}

void gfx::D3D12Device::BeginCapture(std::wstring const& filename)
{
#if ENABLE_PIX_CAPUTRE
...
void gfx::D3D12Device::EndCapture(bool discard)
{
#if ENABLE_PIX_CAPUTRE
	if (this->m_pixCaptureModule)
	{
...
bool gfx::D3D12Device::CheckCapability(DeviceCapability deviceCapability)
{
	return (this->m_capabilities & deviceCapability) == deviceCapability;
}

float gfx::D3D12Device::GetAvgFrameTime()
{
	return 0.0f;
}

bool gfx::D3D12Device::IsDevicedRemoved()
{
	HRESULT hr = this->GetD3D12Device5()->GetDeviceRemovedReason();
	return FAILED(hr);
}

void gfx::D3D12Device::DeleteD3DResource(Microsoft::WRL::ComPtr<ID3D12Resource> resource)
{
	DeleteItem d =
	{
		this->m_frameCount,
...
TextureHandle gfx::D3D12Device::CreateRenderTarget(
...
TextureHandle gfx::D3D12Device::CreateTexture(TextureDesc const& desc, Microsoft::WRL::ComPtr<ID3D12Resource> d3d12Resource)
{
	// Construct Texture
	D3D12Texture texture = {};
	texture.Desc = desc;
	...
	if ((desc.BindingFlags & BindingFlags::ShaderResource) == BindingFlags::ShaderResource)
	{
		this->CreateSubresource(textureHande, gfx::SubresouceType::SRV, 0, ~0u, 0, ~0u);
	}

	if ((desc.BindingFlags & BindingFlags::RenderTarget) == BindingFlags::RenderTarget)
	{
		this->CreateSubresource(textureHande, gfx::SubresouceType::RTV, 0, ~0u, 0, ~0u);
	}

	if ((desc.BindingFlags & BindingFlags::DepthStencil) == BindingFlags::DepthStencil)
	{
		this->CreateSubresource(textureHande, gfx::SubresouceType::DSV, 0, ~0u, 0, ~0u);
	}

	if ((desc.BindingFlags & BindingFlags::UnorderedAccess) == BindingFlags::UnorderedAccess)
	{
		this->CreateSubresource(textureHande, gfx::SubresouceType::UAV, 0, ~0u, 0, ~0u);
	}

	return textureHande;
}

int gfx::D3D12Device::CreateShaderResourceView(TextureHandle texture, uint32_t firstSlice, uint32_t sliceCount, uint32_t firstMip, uint32_t mipCount)
{
...
int gfx::D3D12Device::CreateRenderTargetView(TextureHandle texture, uint32_t firstSlice, uint32_t sliceCount, uint32_t firstMip, uint32_t mipCount)
{
	D3D12Texture* textureImpl = this->m_texturePool.Get(texture);
...
int gfx::D3D12Device::CreateDepthStencilView(TextureHandle texture, uint32_t firstSlice, uint32_t sliceCount, uint32_t firstMip, uint32_t mipCount)
{
	D3D12Texture* textureImpl = this->m_texturePool.Get(texture);
...
int gfx::D3D12Device::CreateUnorderedAccessView(TextureHandle texture, uint32_t firstSlice, uint32_t sliceCount, uint32_t firstMip, uint32_t mipCount)
{
	D3D12Texture* textureImpl = this->m_texturePool.Get(texture);
...
/*
RootSignatureHandle PhxEngine::gfx::Dx12::GraphicsDevice::CreateRootSignature(GraphicsPipelineDesc const& desc)
{
	/*
...
void gfx::D3D12Device::RunGarbageCollection(uint64_t completedFrame)
{
...
void gfx::D3D12Device::TranslateBlendState(BlendRenderState const& inState, D3D12_BLEND_DESC& outState)
{
...
void gfx::D3D12Device::TranslateDepthStencilState(DepthStencilRenderState const& inState, D3D12_DEPTH_STENCIL_DESC& outState)
{
...
void gfx::D3D12Device::TranslateRasterState(RasterRenderState const& inState, D3D12_RASTERIZER_DESC& outState)
{
...
void gfx::D3D12Device::CreateBufferInternal(BufferDesc const& desc, D3D12Buffer& outBuffer)
{
	outBuffer.Desc = desc;

	D3D12_RESOURCE_FLAGS resourceFlags = D3D12_RESOURCE_FLAG_NONE;
	if ((desc.Binding & gfx::BindingFlags::UnorderedAccess) == gfx::BindingFlags::UnorderedAccess)
	{
		resourceFlags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
	}
...
int gfx::D3D12Device::CreateShaderResourceView(BufferHandle buffer, size_t offset, size_t size)
{
	D3D12Buffer* bufferImpl = this->m_bufferPool.Get(buffer);
	...
	if (bufferImpl->Desc.Format == gfx::Format::UNKNOWN)
	{
	...
	const bool isBindless = (bufferImpl->GetDesc().MiscFlags & gfx::BufferMiscFlags::Bindless) == gfx::BufferMiscFlags::Bindless;
	if (isBindless)
	{
		// Copy Descriptor to Bindless since we are creating a texture as a shader resource view
		view.BindlessIndex = this->m_bindlessResourceDescriptorTable.Allocate();
...
int gfx::D3D12Device::CreateUnorderedAccessView(BufferHandle buffer, size_t offset, size_t size)
{
	D3D12Buffer* bufferImpl = this->m_bufferPool.Get(buffer);
...
#if false
void gfx::D3D12Device::CreateGpuTimestampQueryHeap(uint32_t queryCount)
{
...
#endif
void gfx::D3D12Device::InitializeResourcePools()
{
	constexpr uint32_t kInitPoolSize = 16;
	this->m_swapChainPool.Initialize(1);
...
void gfx::D3D12Device::InitializeD3D12NativeResources(IDXGIAdapter* gpuAdapter)
{
	...
	this->m_rootDevice->SetName(L"gfx::D3D12Device::RootDevice");
...
SwapChainHandle gfx::D3D12Device::CreateSwapChain(SwapChainDesc const& desc, void* windowHandle)
{
	// TODO: re work this
	SwapChainHandle handle = this->m_swapChainPool.Emplace();
	D3D12SwapChain& swapChainImpl = *this->m_swapChainPool.Get(handle);
	swapChainImpl.Desc = desc;
	...
	if (swapChainImpl.NativeSwapchain == nullptr)
	{
		// Create swapchain:
		...
		hr = this->m_factory->CreateSwapChainForHwnd(
			...
			&fullscreenDesc,
			nullptr,
			swapChainImpl.NativeSwapchain.GetAddressOf()
		);
		...
		hr = swapChainImpl.NativeSwapchain.As(&swapChainImpl.NativeSwapchain4);
		if (FAILED(hr))
		{
			throw std::exception();
		}
	...
	else
	{
		...
		// Delete back buffers
		for (int i = 0; i < swapChainImpl.BackBuffers.size(); i++)
		{
			this->DeleteTexture(swapChainImpl.BackBuffers[i]);
		}

		this->RunGarbageCollection(this->m_frameCount + 1);
		swapChainImpl.BackBuffers.clear();

		hr = swapChainImpl.NativeSwapchain4->ResizeBuffers(
			desc.BufferCount,
			desc.Width,
			desc.Height,
	...
	swapChainImpl.BackBuffers.resize(desc.BufferCount);

	for (UINT i = 0; i < swapChainImpl.Desc.BufferCount; i++)
	{
		Microsoft::WRL::ComPtr<ID3D12Resource> backBuffer;
		ThrowIfFailed(
			swapChainImpl.NativeSwapchain4->GetBuffer(i, IID_PPV_ARGS(&backBuffer)));

		char allocatorName[32];
		sprintf_s(allocatorName, "Back Buffer %iu", i);

		swapChainImpl.BackBuffers[i] = this->CreateTexture(
			{
				.BindingFlags = BindingFlags::RenderTarget,
				.Dimension = TextureDimension::Texture2D,
				.Format = swapChainImpl.Desc.Format,
				.Width = swapChainImpl.Desc.Width,
				.Height = swapChainImpl.Desc.Height,
...
void gfx::D3D12Device::FindAdapter(Microsoft::WRL::ComPtr<IDXGIFactory6> factory, D3D12GpuAdapter& outAdapter)
{
	...
	for (uint32_t adapterIndex = 0; D3D12GpuAdapter::EnumAdapters(adapterIndex, factory.Get(), tempAdapter.ReleaseAndGetAddressOf()) != DXGI_ERROR_NOT_FOUND; ++adapterIndex)
	{
		if (!tempAdapter)
		{
			continue;
	...
	outAdapter.PlatformDesc = desc;
	outAdapter.PlatformAdapter = selectedAdapter;
}
...
#if false
CommandListHandle gfx::D3D12Device::BeginCommandList(CommandQueueType queueType)
{
	...
	if (cmdList.NativeCommandList == nullptr)
	{
		...
		cmdList.NativeCommandList->SetName(L"gfx::D3D12Device::CommandList");
		ThrowIfFailed(
...
void gfx::D3D12Device::WaitCommandList(CommandListHandle cmd, CommandListHandle waitOn)
{
...
void gfx::D3D12Device::RTBuildAccelerationStructure(gfx::RTAccelerationStructureHandle accelStructure, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
		...
		for (int i = 0; i < dx12AccelStructure->Desc.ButtomLevel.Geometries.size(); i++)
		{
			...
			if (geometry.Type == RTAccelerationStructureDesc::BottomLevelDesc::Geometry::Type::Triangles)
			{
				...
				buildGeometry.Triangles.IndexBuffer = indexBuffer->D3D12Resource->GetGPUVirtualAddress() +
					(D3D12_GPU_VIRTUAL_ADDRESS)geometry.Triangles.IndexOffset * (geometry.Triangles.IndexFormat == gfx::Format::R16_UINT ? sizeof(uint16_t) : sizeof(uint32_t));
...
void gfx::D3D12Device::BeginMarker(std::string_view name, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	PIXBeginEvent(internalCmd.NativeCommandList.Get(), 0, std::wstring(name.begin(), name.end()).c_str());
}

void gfx::D3D12Device::EndMarker(CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	PIXEndEvent(internalCmd.NativeCommandList.Get());
}

void gfx::D3D12Device::TransitionBarrier(
...
void gfx::D3D12Device::TransitionBarriers(Span<GpuBarrier> gpuBarriers, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	for (const phx::gfx::GpuBarrier& gpuBarrier : gpuBarriers)
	{
...
void gfx::D3D12Device::ClearTextureFloat(TextureHandle texture, Color const& clearColour, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
GPUAllocation gfx::D3D12Device::AllocateGpu(size_t bufferSize, size_t stride, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::ClearDepthStencilTexture(
...
void gfx::D3D12Device::Draw(DrawArgs const& args, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::DrawIndexed(DrawArgs const& args, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::ExecuteIndirect(gfx::CommandSignatureHandle commandSignature, gfx::BufferHandle args, size_t argsOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::ExecuteIndirect(
	gfx::CommandSignatureHandle commandSignature,
	gfx::BufferHandle args,
	size_t argsOffsetInBytes,
	gfx::BufferHandle count,
...
void gfx::D3D12Device::DrawIndirect(gfx::BufferHandle args, size_t argsOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* bufferImpl = this->GetBufferPool().Get(args);
	internalCmd.NativeCommandList6->ExecuteIndirect(
...
void gfx::D3D12Device::DrawIndirect(gfx::BufferHandle args, size_t argsOffsetInBytes, PHX_UNUSED gfx::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* argBufferImpl = this->GetBufferPool().Get(args);
...
void gfx::D3D12Device::DrawIndexedIndirect(gfx::BufferHandle args, size_t argsOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* bufferImpl = this->GetBufferPool().Get(args);
	internalCmd.NativeCommandList6->ExecuteIndirect(
...
void gfx::D3D12Device::DrawIndexedIndirect(gfx::BufferHandle args, size_t argsOffsetInBytes, PHX_UNUSED gfx::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* argBufferImpl = this->GetBufferPool().Get(args);
	D3D12Buffer* countBufferBufferImpl = this->GetBufferPool().Get(args);
...
void gfx::D3D12Device::WriteBuffer(BufferHandle buffer, const void* Data, size_t dataSize, uint64_t destOffsetBytes, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::CopyBuffer(BufferHandle dst, uint64_t dstOffset, BufferHandle src, uint64_t srcOffset, size_t sizeInBytes, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::WriteTexture(TextureHandle texture, uint32_t firstSubresource, size_t numSubresources, SubresourceData* pSubresourceData, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	auto textureImpl = this->GetTexturePool().Get(texture);
...
void gfx::D3D12Device::WriteTexture(PHX_UNUSED TextureHandle texture, PHX_UNUSED uint32_t arraySlice, PHX_UNUSED uint32_t mipLevel, PHX_UNUSED const void* Data, PHX_UNUSED size_t rowPitch, PHX_UNUSED size_t depthPitch, PHX_UNUSED CommandListHandle cmd)
{
#if false
...
void gfx::D3D12Device::SetRenderTargets(Span<TextureHandle> renderTargets, TextureHandle depthStencil, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::SetGfxPipeline(GfxPipelineHandle graphicsPiplineHandle, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::SetViewports(Viewport* viewports, size_t numViewports, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::SetScissors(Rect* scissors, size_t numScissors, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindPushConstant(uint32_t rootParameterIndex, uint32_t sizeInBytes, const void* constants, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindConstantBuffer(size_t rootParameterIndex, BufferHandle constantBuffer, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindDynamicConstantBuffer(size_t rootParameterIndex, size_t sizeInBytes, const void* bufferData, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindVertexBuffer(uint32_t slot, BufferHandle vertexBuffer, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindDynamicVertexBuffer(uint32_t slot, size_t numVertices, size_t vertexSize, const void* vertexBufferData, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindIndexBuffer(BufferHandle indexBuffer, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindDynamicIndexBuffer(size_t numIndicies, gfx::Format indexFormat, const void* indexBufferData, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	size_t indexSizeInBytes = indexFormat == gfx::Format::R16_UINT ? 2 : 4;
...
void gfx::D3D12Device::BindDynamicStructuredBuffer(uint32_t rootParameterIndex, size_t numElements, size_t elementSize, const void* bufferData, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindStructuredBuffer(size_t rootParameterIndex, BufferHandle buffer, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	const D3D12Buffer* bufferImpl = this->GetBufferPool().Get(buffer);
...
void gfx::D3D12Device::BindResourceTable(size_t rootParameterIndex, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindSamplerTable(PHX_UNUSED size_t rootParameterIndex, PHX_UNUSED CommandListHandle cmd)
{
	// TODO:
	assert(false);
}

void gfx::D3D12Device::BindDynamicDescriptorTable(size_t rootParameterIndex, Span<TextureHandle> textures, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::BindDynamicUavDescriptorTable(
...
void gfx::D3D12Device::SetComputeState(ComputePipelineHandle state, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::Dispatch(uint32_t groupsX, uint32_t groupsY, uint32_t groupsZ, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	internalCmd.NativeCommandList->Dispatch(groupsX, groupsY, groupsZ);
}

void gfx::D3D12Device::DispatchIndirect(gfx::BufferHandle args, uint32_t argsOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* bufferImpl = this->GetBufferPool().Get(args);
	internalCmd.NativeCommandList6->ExecuteIndirect(
...
void gfx::D3D12Device::DispatchIndirect(gfx::BufferHandle args, uint32_t argsOffsetInBytes, PHX_UNUSED gfx::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* argBufferImpl = this->GetBufferPool().Get(args);
	D3D12Buffer* countBufferBufferImpl = this->GetBufferPool().Get(args);
...
void gfx::D3D12Device::SetMeshPipeline(MeshPipelineHandle meshPipeline, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::DispatchMesh(uint32_t groupsX, uint32_t groupsY, uint32_t groupsZ, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::DispatchMeshIndirect(gfx::BufferHandle args, uint32_t argsOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* bufferImpl = this->GetBufferPool().Get(args);
	internalCmd.NativeCommandList6->ExecuteIndirect(
...
void gfx::D3D12Device::DispatchMeshIndirect(gfx::BufferHandle args, uint32_t argsOffsetInBytes, PHX_UNUSED gfx::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12Buffer* argBufferImpl = this->GetBufferPool().Get(args);
	D3D12Buffer* countBufferBufferImpl = this->GetBufferPool().Get(args);
...
void gfx::D3D12Device::BeginTimerQuery(TimerQueryHandle query, CommandListHandle cmd)
{
	assert(query.IsValid());
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
...
void gfx::D3D12Device::EndTimerQuery(TimerQueryHandle query, CommandListHandle cmd)
{
	D3D12CommandList& internalCmd = this->m_commandListPool[cmd];
	D3D12TimerQuery* queryImpl = this->GetTimerQueryPool().Get(query);
