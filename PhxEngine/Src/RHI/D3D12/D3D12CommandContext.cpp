...
PhxEngine::RHI::D3D12::D3D12CommandContext::D3D12CommandContext(D3D12CommandQueue* parentQueue)
	: m_parentQueue(parentQueue)
{
}

PhxEngine::RHI::D3D12::D3D12CommandContext::~D3D12CommandContext()
{
    this->m_d3d12CommandList.Reset();
}

void PhxEngine::RHI::D3D12::D3D12CommandContext::Open()
{
    ...
    {
        D3D12Driver::GPtr->GetD3D12Device()->CreateCommandList(
            0,
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::Close()
{
    // Is this needed
...
ScopedMarker D3D12CommandContext::BeginScopedMarker(std::string_view name)
{
    this->BeginMarker(name);
    return ScopedMarker(this);
}

void D3D12CommandContext::BeginMarker(std::string_view name)
{
    PIXBeginEvent(this->m_d3d12CommandList.Get(), 0, std::wstring(name.begin(), name.end()).c_str());
}

void D3D12CommandContext::EndMarker()
{
    PIXEndEvent(this->m_d3d12CommandList.Get());
}


void D3D12CommandContext::Executed(uint64_t fenceValue)
{
...
#if flase
void PhxEngine::RHI::D3D12::D3D12CommandContext::RTBuildAccelerationStructure(RHI::RTAccelerationStructureHandle accelStructure)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::TransitionBarrier(
    BufferHandle buffer,
    ResourceStates beforeState,
    ResourceStates afterState)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::TransitionBarriers(Core::Span<GpuBarrier> gpuBarriers)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BeginRenderPassBackBuffer(bool clear)
{
...
RenderPassHandle PhxEngine::RHI::D3D12::D3D12CommandContext::GetRenderPassBackBuffer()
{
    D3D12Viewport& viewport = *this->m_graphicsDevice.GetActiveViewport();
    return viewport.RenderPass;
}

void PhxEngine::RHI::D3D12::D3D12CommandContext::BeginRenderPass(RenderPassHandle renderPass)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::EndRenderPass()
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::ExecuteIndirect(RHI::CommandSignatureHandle commandSignature, RHI::BufferHandle args, size_t argsOffsetInBytes, uint32_t maxCount)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::ExecuteIndirect(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::DrawIndirect(RHI::BufferHandle args, size_t argsOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* bufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    this->m_d3d12CommandList6->ExecuteIndirect(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::DrawIndirect(RHI::BufferHandle args, size_t argsOffsetInBytes, RHI::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* argBufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::DrawIndexedIndirect(RHI::BufferHandle args, size_t argsOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* bufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    this->m_d3d12CommandList6->ExecuteIndirect(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::DrawIndexedIndirect(RHI::BufferHandle args, size_t argsOffsetInBytes, RHI::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* argBufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    D3D12Buffer* countBufferBufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    this->m_d3d12CommandList6->ExecuteIndirect(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::WriteTexture(TextureHandle texture, uint32_t firstSubresource, size_t numSubresources, SubresourceData* pSubresourceData)
{
    auto textureImpl = this->m_graphicsDevice.GetTexturePool().Get(texture);
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::WriteTexture(TextureHandle texture, uint32_t arraySlice, uint32_t mipLevel, const void* Data, size_t rowPitch, size_t depthPitch)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::SetRenderTargets(std::vector<TextureHandle> const& renderTargets, TextureHandle depthStencil)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::SetGraphicsPipeline(GraphicsPipelineHandle graphicsPiplineHandle)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::SetViewports(Viewport* viewports, size_t numViewports)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::SetScissors(Rect* scissors, size_t numScissors)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::SetDescritporHeaps(std::array<GpuDescriptorHeap*, 2> const& shaderHeaps)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindPushConstant(uint32_t rootParameterIndex, uint32_t sizeInBytes, const void* constants)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindConstantBuffer(size_t rootParameterIndex, BufferHandle constantBuffer)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindDynamicConstantBuffer(size_t rootParameterIndex, size_t sizeInBytes, const void* bufferData)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindVertexBuffer(uint32_t slot, BufferHandle vertexBuffer)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindDynamicVertexBuffer(uint32_t slot, size_t numVertices, size_t vertexSize, const void* vertexBufferData)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindIndexBuffer(BufferHandle indexBuffer)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindDynamicIndexBuffer(size_t numIndicies, RHIFormat indexFormat, const void* indexBufferData)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindDynamicStructuredBuffer(uint32_t rootParameterIndex, size_t numElements, size_t elementSize, const void* bufferData)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindStructuredBuffer(size_t rootParameterIndex, BufferHandle buffer)
{
    const D3D12Buffer* bufferImpl = this->m_graphicsDevice.GetBufferPool().Get(buffer);
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindResourceTable(size_t rootParameterIndex)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindSamplerTable(size_t rootParameterIndex)
{
    // TODO:
    assert(false);
}
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BindDynamicUavDescriptorTable(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::SetComputeState(ComputePipelineHandle state)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::Dispatch(uint32_t groupsX, uint32_t groupsY, uint32_t groupsZ)
{
    this->m_d3d12CommandList->Dispatch(groupsX, groupsY, groupsZ);
}

void PhxEngine::RHI::D3D12::D3D12CommandContext::DispatchIndirect(RHI::BufferHandle args, uint32_t argsOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* bufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    this->m_d3d12CommandList6->ExecuteIndirect(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::DispatchIndirect(RHI::BufferHandle args, uint32_t argsOffsetInBytes, RHI::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* argBufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    D3D12Buffer* countBufferBufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    this->m_d3d12CommandList6->ExecuteIndirect(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::SetMeshPipeline(MeshPipelineHandle meshPipeline)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::DispatchMesh(uint32_t groupsX, uint32_t groupsY, uint32_t groupsZ)
{
    assert(this->m_activePipelineType == PipelineType::Mesh);
    this->m_d3d12CommandList6->DispatchMesh(groupsX, groupsY, groupsZ);
}

void PhxEngine::RHI::D3D12::D3D12CommandContext::DispatchMeshIndirect(RHI::BufferHandle args, uint32_t argsOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* bufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    this->m_d3d12CommandList6->ExecuteIndirect(
        this->m_graphicsDevice.GetDispatchIndirectCommandSignature(),
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::DispatchMeshIndirect(RHI::BufferHandle args, uint32_t argsOffsetInBytes, RHI::BufferHandle count, size_t countOffsetInBytes, uint32_t maxCount)
{
    D3D12Buffer* argBufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    D3D12Buffer* countBufferBufferImpl = this->m_graphicsDevice.GetBufferPool().Get(args);
    this->m_d3d12CommandList6->ExecuteIndirect(
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::BeginTimerQuery(TimerQueryHandle query)
{
...
void PhxEngine::RHI::D3D12::D3D12CommandContext::EndTimerQuery(TimerQueryHandle query)
{
    D3D12TimerQuery* queryImpl = this->m_graphicsDevice.GetTimerQueryPool().Get(query);
