...
{
    PHX_LOG_INFO(Log::Channels::RHI, "Initializing Descriptor System system");
    
    PHX_LOG_INFO(Log::Channels::RHI, "Initializing resource heap of {0} descriptors", max_resource_descriptors);
    resource_heap.Initialize(vk_device, vma_allocator, vk_physical_device, vulkan::HeapType::Resource, max_resource_descriptors);

    PHX_LOG_INFO(Log::Channels::RHI, "Initializing sampler heap of {0} descriptors", max_sampler_descriptors);
...
void phx::rhi::vulkan::DescriptorSystem::CreateMasterPipelineLayout(VkDevice vk_device)
{
    ...
    PHX_LOG_INFO(Log::Channels::RHI, "Max push constant size per shader is {0}", max_push_constant_size);
