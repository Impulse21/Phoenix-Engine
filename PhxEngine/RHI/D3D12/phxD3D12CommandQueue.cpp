...
void phx::rhi::d3d12::D3D12CommandQueue::Finailize()
{
...
ID3D12CommandAllocator* phx::rhi::d3d12::D3D12CommandQueue::RequestAllocator()
{
	auto lastCompletedFence = this->GetLastCompletedFence();
	return this->m_allocatorPool.RequestAllocator(lastCompletedFence);
}

void phx::rhi::d3d12::D3D12CommandQueue::DiscardAllocators(uint64_t fence, Span<ID3D12CommandAllocator*> allocators)
{
...
void phx::rhi::d3d12::D3D12CommandQueue::DiscardAllocator(uint64_t fence, ID3D12CommandAllocator* allocator)
{
	this->m_allocatorPool.DiscardAllocator(fence, allocator);
}

uint64_t phx::rhi::d3d12::D3D12CommandQueue::IncrementFence()
{
...
bool phx::rhi::d3d12::D3D12CommandQueue::IsFenceComplete(uint64_t fenceValue)
{
...
void phx::rhi::d3d12::D3D12CommandQueue::WaitForFence(uint64_t fenceValue)
{
...
uint64_t phx::rhi::d3d12::D3D12CommandQueue::GetLastCompletedFence()
{
	std::scoped_lock _(this->m_fenceMutex);
	return this->m_d3d12Fence->GetCompletedValue();
}

void phx::rhi::d3d12::D3D12CommandQueue::CommandAllocatorPool::Initialize(ID3D12Device* nativeDevice, D3D12_COMMAND_LIST_TYPE type)
{
	this->m_type = type;
	this->m_nativeDevice = nativeDevice;
}

ID3D12CommandAllocator* phx::rhi::d3d12::D3D12CommandQueue::CommandAllocatorPool::RequestAllocator(uint64_t completedFenceValue)
{
...
void phx::rhi::d3d12::D3D12CommandQueue::CommandAllocatorPool::DiscardAllocator(uint64_t fence, ID3D12CommandAllocator* allocator)
{
	std::lock_guard<std::mutex> lockGuard(this->m_allocatonMutex);
