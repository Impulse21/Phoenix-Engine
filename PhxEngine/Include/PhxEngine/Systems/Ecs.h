...
namespace PhxEngine::ECS
{
	...
	public:
		...
		TComponent& Create(Entity entity)
		{
			...
			assert(this->m_entities.size() == this->m_components.size());
			assert(this->m_lookup.size() == this->m_components.size());
		...
		{
			assert(indexFrom < this->GetCount());
			assert(indexTo < this->GetCount());
