...
std::unique_ptr<DirectX::ScratchImage> PhxEngine::Pipeline::DDSTextureConverter::BuildDDS(IFileSystem* fileSystem, std::string const& filename, TexConversionFlags flags)
{
    std::unique_ptr<IBlob> blob = fileSystem->ReadFile(filename);
    PHX_LOG_INFO("Converting file '%s' to DDS.", filename.c_str());
    return DDSTextureConverter::BuildDDS(blob.get(), GetTextureType(FileSystem::GetFileExt(filename)), flags);
}


std::unique_ptr<DirectX::ScratchImage> PhxEngine::Pipeline::DDSTextureConverter::BuildDDS(IBlob* blob, TextureType type, TexConversionFlags flags)
{
    ...
    case TextureType::DDS:
        hr = DirectX::LoadFromDDSMemory(blob->Data(), blob->Size(), DirectX::DDS_FLAGS_NONE, &info, *image);
        if (FAILED(hr))
        {
    ...
    case TextureType::TGA:
        hr = DirectX::LoadFromTGAMemory(blob->Data(), blob->Size(), &info, *image);
        if (FAILED(hr))
        {
    ...
    case TextureType::HDR:
        hr = DirectX::LoadFromHDRMemory(blob->Data(), blob->Size(), &info, *image);
        if (FAILED(hr))
        {
    ...
    case TextureType::WIC:
    {
        ...
        hr = DirectX::LoadFromWICMemory(blob->Data(), blob->Size(), wicFlags, &info, *image);
        if (FAILED(hr))
        {
            PHX_LOG_ERROR("Could not load texture HDR texture from memory (DDS: %08X).", hr);
            return {};
