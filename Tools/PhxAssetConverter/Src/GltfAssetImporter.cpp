...
bool PhxEngine::Pipeline::GltfAssetImporter::Import(IFileSystem* fileSystem, std::string const& filename, ImportedObjects& importedObjects)
{
	importedObjects = {};
...
bool PhxEngine::Pipeline::GltfAssetImporter::ImportTexture(Pipeline::CgltfContext const& cgltfContext, cgltf_texture* gltfTexture, Pipeline::Texture& outTexture)
{
	...
	if (gltfTexture->image->buffer_view)
	{
		...
		outTexture.DataBlob = FileSystemFactory::CreateBlob(dataPtr, dataSize);
	}
