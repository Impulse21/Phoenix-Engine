...
namespace PhxEngine::Pipeline
{
	...
	{
	public:
        static std::unique_ptr<DirectX::ScratchImage> BuildDDS(Core::IFileSystem* fileSystem, std::string const& filename, TexConversionFlags flags);
        static std::unique_ptr<DirectX::ScratchImage> BuildDDS(Core::IBlob* blob, TextureType type, TexConversionFlags flags);
	};
}
