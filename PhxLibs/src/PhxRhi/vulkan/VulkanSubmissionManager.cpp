...
{
    {
        ...
        {
            VkResult result = vkCreateSemaphore(vulkan_backend->vk_device, &semaphore_info, nullptr, &frames[i].PresentSemaphore);
            PHX_CORE_ASSERT(result == VK_SUCCESS);

            result = vkCreateSemaphore(vulkan_backend->vk_device, &semaphore_info, nullptr, &frames[i].RenderSemaphore);
            PHX_CORE_ASSERT(result == VK_SUCCESS);

            result = vkCreateFence(vulkan_backend->vk_device, &fence_info, nullptr, &frames[i].);
            PHX_CORE_ASSERT(result == VK_SUCCESS);
        }
...
void phx::rhi::VulkanSubmissionManager::Shutdown()
{
    ...
    for (size_t i = 0; i < cMaxInflightFrames; ++i)
    {
        ...
        {
            vkDestroyFence(vulkan_backend->vk_device, VkContext::frames[i].RenderFence, nullptr);
            VkContext::frames[i].RenderFence = VK_NULL_HANDLE;
        }
        if (VkContext::frames[i].RenderSemaphore != VK_NULL_HANDLE)
        {
            vkDestroySemaphore(vulkan_backend->vk_device, VkContext::frames[i].RenderSemaphore, nullptr);
            VkContext::frames[i].RenderSemaphore = VK_NULL_HANDLE;
        }
        if (VkContext::frames[i].PresentSemaphore != VK_NULL_HANDLE)
        {
            vkDestroySemaphore(vulkan_backend->vk_device, VkContext::frames[i].PresentSemaphore, nullptr);
...
{
    PHX_CORE_ASSERT(VkContext::is_initialized && vulkan_backend->vk_device != VK_NULL_HANDLE);
