...
class DirectLightingApp : public ApplicationBase
{
private:

public:
    DirectLightingApp(IPhxEngineRoot* root)
    ...
    void Update(Core::TimeStep const& deltaTime) override
    {
        ...
        if (this->IsSceneLoaded())
        {
            ...
            if (lightComp.IsEnabled() && lightComp.Type != Scene::LightComponent::kDirectionalLight)
            {
                ...
                debugInstance->EmissiveColor = lightComp.Colour;
...
class DirectLightingAppUI : public PhxEngine::ImGuiRenderer
{
private:

public:
    DirectLightingAppUI(IPhxEngineRoot* root, DirectLightingApp* app)
        : ImGuiRenderer(root)
        , m_app(app)
    {
    }
...
#endif
{
    ...
    {
        DirectLightingApp app(root.get());
        if (app.Initialize())
        {
            root->AddPassToBack(&app);

            DirectLightingAppUI userInterface(root.get(), &app);
